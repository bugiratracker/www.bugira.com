...
// Config
var Config = {
  PLUGINS_LOCALPATH: "./assets/build/js/plugins/"
...
(function ($) {
  $.extend($.fn, {
    ...
    // ----------------------------------------------------------------
    isPageLoaderDone: function isPageLoaderDone(callback) {
    ...
    // ----------------------------------------------------------------
    includeWaypoints: function includeWaypoints(callback) {
    ...
    // ----------------------------------------------------------------
    elementInView: function elementInView(obj) {
    ...
    themeScrollMenus: function themeScrollMenus() {
      ...
      if (scrollLinks.length > 0) {
        var getHeaderOffset = function getHeaderOffset(extra) {
        ...
        var customActiveSpyClass = function customActiveSpyClass(active) {
        ...
        var triggerSpy = function triggerSpy(state) {
          var offsetExtra = 5;
        ...
        triggerSpy('init'); // Custom active class per link
    ...
    themeSubMenus: function themeSubMenus() {
      ...
      context.find('.dropdown-menu [data-toggle=dropdown]').on('click', function (event) {
        event.preventDefault();
        event.stopPropagation(); // Toggle direct parent
      ...
      context.find('.dropdown.dropdown-persist').on({
        ...
        "hide.bs.dropdown": function hideBsDropdown(event) {
    ...
    calcHeightsOffset: function calcHeightsOffset(height, offset) {
    ...
    // Load plugin
    // --------------------------------
    themeLoadPlugin: function themeLoadPlugin(js, css, callback, placement) {
      ...
      var themeLoadPluginPath = function themeLoadPluginPath(url) {
      ...
      if (js.length > 0) {
        ...
        var internalCallback = function internalCallback(url) {
          // Complete
...
})(jQuery); // Temp fix for https://github.com/vmitsaras/js-offcanvas/commit/ab116f632a092477574d69760ecbe3d7c07b00ab#diff-7399a891b21afad97155db02f6dbd60dL142
...
(function ($) {
  $.extend($.fn, {
    themeCustomScripts: function themeCustomScripts(refresh) {
      var context = $(this);
      ...
      $document = $(document); // Menus need init quick, @see utils.js
      ...
      if (elementsHovered.length > 0) {
        var initElementsHovered = function initElementsHovered() {
          elementsHovered.each(function () {
            var $element = jQuery(this),
                ...
                animateDuration = $element.data('hover-duration') || null; // Delays & durations
      ...
      if (fullHeights.length > 0) {
        var doFullHeightsOffset = function doFullHeightsOffset(height, offset) {
          return $document.calcHeightsOffset(height, offset);
        };

        var doFullHeights = function doFullHeights() {
      ...
      if (elementsAnimated.length > 0) {
        var initElementsAnimated = function initElementsAnimated() {
          ...
          elementsAnimated.each(function () {
            var $element = $(this),
                ...
                elementInView = $document.elementInView($element); // Already scroll passed so don't animate again
      ...
      context.find('[data-scroll="scroll-state"]').each(function () {
        var $scroll = $(this),
            ...
            scrollAmountOut = $scroll.data('scroll-amount-out') || null,
            //@todo
      ...
      context.find('[data-toggle="quantity"]').each(function () {
        ...
        var toggleQuantity = function toggleQuantity(direction) {
      ...
      context.find('[data-css]').each(function () {
        ...
        if (styleProps !== null && _typeof(styleProps) === 'object') {
      ...
      if (overlayMenus.length > 0) {
        overlayMenus.each(function () {
          var $this = jQuery(this),
              target = $this.data('target') || null; // General class for all triggers
        ...
        context.find('[data-dismiss="overlay"]').each(function () {
          var $this = jQuery(this),
              ...
              $trigger = jQuery('[data-toggle="overlay"][data-target="' + $target + '"]') || null; // Check target overlay to close exists

          if ($($target).length > 0) {
            $target = jQuery($target);
            $this.on('click', function (e) {
              $target.removeClass('overlay-active');
              jQuery('html').removeClass('overlay-open'); // Try to find the trigger
      ...
      context.find('[data-url]').each(function () {
        ...
        var parseStringUrl = function parseStringUrl(url) {
        ...
        $(this).addClass('clickable-element'); // Hover event
      ...
      if (progressBarsAnimated.length > 0) {
        var initProgressBarsAnimated = function initProgressBarsAnimated() {
      ...
      if (modalsDuration.length > 0) {
        ...
        $modal.find('.modal-content').append(progressBar); // Actual durations
      ...
      if (modalsOnload.length > 0) {
        ...
        modalsOnload.each(function () {
          ...
          var startModal = function startModal($modal) {
            var shown = $modal.data('modal-shown') || false; //if (shown === false || shown && force) {
      ...
      context.find('[data-toggle="class"]').each(function () {
        ...
        var toggleCallback = function toggleCallback(t, c, a) {
          if (_typeof(c) === 'object') {
        ...
        if (_typeof(target) === 'object') {
          $.each(target, function (selector, data) {
            ...
            $this.on(_toggleTrigger, function () {
              if (_typeof(_toggleActions) === 'object') {
      ...
      context.find('[data-clone]').each(function () {
        ...
        var classesAdd = $this.data('clone-classes-add') || []; // Clone to
        ...
        $clone = $clone.clone();
        $clone.addClass('cloned-element'); // Remove elements
        // Classes
...
})(jQuery); // Object of plugins to add to Globals.PLUGINS
// Add custom plugin integration here or as separate files
// within the /src/plugins directory


var defaultPlugins = {//themePluginPLUGINNAME: function(context) {
...
Globals.PLUGINS.themePluginBackstretch = function (context) {
  ...
  if ($backstretches.length > 0) {
    var themePluginBackstretchInit = function themePluginBackstretchInit() {
      $backstretches.each(function () {
        ...
        if (backstretchImgs) {
          $('html').addClass('has-backstretch'); // Merge in any custom settings

          backstretchSettings = $.extend({}, backstretchSettings, backstretchEl.data());
          backstretchTarget.backstretch(backstretchImgs, backstretchSettings); // add overlay
...
Globals.PLUGINS.themePluginBlazy = function (context) {
  ...
  if ($blazys.length > 0 || $blazy_bgs.length > 0) {
    var themePluginBlazyInit = function themePluginBlazyInit() {
      if ($blazys.length > 0) {
    ...
    $document.themeLoadPlugin(["https://cdn.jsdelivr.net/blazy/latest/blazy.min.js"], ["plugin-css/plugin-blazy.min.css"], themePluginBlazyInit);
  }
}; // Object of plugins to add to Globals.PLUGINS
...
Globals.PLUGINS.themePluginBootstrapSelect = function (context) {
  ...
  var themePluginBootstrapSelectInit = function themePluginBootstrapSelectInit() {
    if ($selects.length > 0) {
      $selects.each(function () {
        ...
        if (settings.classes !== null) {
          if (_typeof(settings.classes) == 'object') {
  ...
  if ($selects.length > 0) {
    $document.themeLoadPlugin(["https://cdnjs.cloudflare.com/ajax/libs/bootstrap-select/1.13.0-beta/js/bootstrap-select.min.js"], ["https://cdnjs.cloudflare.com/ajax/libs/bootstrap-select/1.13.0-beta/css/bootstrap-select.min.css", "plugin-css/plugin-bootstrap-select.min.css"], themePluginBootstrapSelectInit);
  }
}; // Object of plugins to add to Globals.PLUGINS
...
Globals.PLUGINS.themePluginBootstrapSwitch = function (context) {
  ...
  if ($bootstrapSwitches.length > 0) {
    var themePluginBootstrapSwitchInit = function themePluginBootstrapSwitchInit() {
      $bootstrapSwitches.bootstrapSwitch();
    };

    $document.themeLoadPlugin(["bootstrap-switch/build/js/bootstrap-switch.min.js"], ["plugin-css/plugin-bootstrap-switch.min.css", "bootstrap-switch/build/css/bootstrap3/bootstrap-switch.min.css"], themePluginBootstrapSwitchInit);
  }
};

var clipboardSelectors = '[data-clipboard-target], [data-clipboard-text]'; // Object of plugins to add to Globals.PREINITPLUGINS
...
Globals.PLUGINS.themePluginClipboard = function (context) {
  ...
  var formatFactory = function formatFactory(html) {
    var parse = function parse(html, tab) {
      if (!tab) {
        tab = 0;
      }
  ...
  var themePluginClipboardInit = function themePluginClipboardInit() {
  ...
  if ($clipboards.length > 0) {
    $document.themeLoadPlugin(["https://cdnjs.cloudflare.com/ajax/libs/clipboard.js/1.7.1/clipboard.min.js"], [], themePluginClipboardInit);
  }
}; // Object of plugins to add to Globals.PLUGINS
...
Globals.PLUGINS.themePluginCountTo = function (context) {
  ...
  if ($countTos.length > 0) {
    var themePluginCountToInit = function themePluginCountToInit() {
...
Globals.PLUGINS.themePluginCountDown = function (context) {
  ...
  if ($countdowns.length > 0) {
    var themePluginCountdownInit = function themePluginCountdownInit() {
      $countdowns.each(function () {
        ...
        $this.countdown(countTo).on('update.countdown', function (event) {
...
Globals.PLUGINS.themePluginCubePortfolio = function (context) {
  ...
  var themePluginCubePortfolioInit = function themePluginCubePortfolioInit() {
    $cubePortfolios.each(function () {
      var $this = $(this),
          customSettings = $this.data('settings') || {},
          settings = $.extend({}, defaultSettings, customSettings);
      $this.data('cbp-settings', settings); // Custom callbacks
      ...
      settings.singlePageInlineCallback = function (url, element) {
        var t = this,
            ...
            customNavButtons = function customNavButtons($html, t) {
        ...
        if (content !== 'ajax' && $(content).length > 0) {
          ...
          $.ajax({
            ...
            t.cubeportfolio.$obj.trigger('updateSinglePageInlineStart.cbp'); //t.singlePageInlineIsOpen.call(t);
      ...
      $this.data('cbp-settings-full', settings); // If imagesLoaded avaliable use it
...
Globals.PLUGINS.themePluginFakeLoader = function (context) {
  ...
  if ($fakeLoaders.length > 0) {
    ...
    var themePluginFakeLoaderInit = function themePluginFakeLoaderInit() {
      ...
      $pageLoader.fakeLoader(options); //$('body').removeClass('page-loader-spacer');
    ...
    $document.themeLoadPlugin(["fakeLoader/fakeLoader.js"], ["fakeLoader/fakeLoader.css"], themePluginFakeLoaderInit);
  }
}; // Object of plugins to add to Globals.PLUGINS
...
Globals.PLUGINS.themePluginFitVids = function (context) {
  ...
  if ($fitVids.length > 0) {
    var themePluginFitVidsInit = function themePluginFitVidsInit() {
      $('body').fitVids({
        ignore: '.no-fitvids'
      });
    };
...
Globals.PLUGINS.themePluginFixTo = function (context) {
  ...
  if ($fixTos.length > 0) {
    var themePluginFixToInit = function themePluginFixToInit() {
      var stickySetSettings = function stickySetSettings(sticky) {
        var stickySettings = {};
      ...
      var stickyStart = function stickyStart(sticky, state) {
        ...
        sticky.addClass('sticky').fixTo(stickyParent, stickySettings); // Sticky from the start - @todo
      ...
      var stickySetPersist = function stickySetPersist(sticky, stickySettings) {
...
Globals.PLUGINS.themePluginFlexslider = function (context) {
  ...
  if ($flexsliders.length > 0) {
    var themePluginFlexsliderInit = function themePluginFlexsliderInit() {
...
Globals.PLUGINS.themePluginHighlightJS = function (context) {
  ...
  if ($highlightJSs.length > 0) {
    var themePluginHighlightJSInit = function themePluginHighlightJSInit() {
      $('pre code').each(function (i, block) {
        hljs.highlightBlock(block);
      });
    };

    $document.themeLoadPlugin(["https://cdnjs.cloudflare.com/ajax/libs/highlight.js/9.12.0/highlight.min.js"], ["https://cdnjs.cloudflare.com/ajax/libs/highlight.js/9.12.0/styles/default.min.css", "https://cdnjs.cloudflare.com/ajax/libs/highlight.js/9.12.0/styles/github.min.css"], themePluginHighlightJSInit);
  }
}; // Object of plugins to add to Globals.PLUGINS
...
Globals.PLUGINS.themePluginIsotope = function (context) {
  ...
  if ($isoTopes.length > 0) {
    var themePluginIsotopeInit = function themePluginIsotopeInit() {
      $isoTopes.each(function () {
        var $container = $(this),
            options = $container.data('isotope-options'),
            filters = $container.data('isotope-filter') || null; // If imagesLoaded avaliable use it
    ...
    $document.themeLoadPlugin(["https://unpkg.com/imagesloaded@4/imagesloaded.pkgd.min.js", "https://unpkg.com/isotope-layout@3/dist/isotope.pkgd.min.js"], [], themePluginIsotopeInit);
  }
}; // Object of plugins to add to Globals.PLUGINS
...
Globals.PLUGINS.themePluginMagnificPopup = function (context) {
  ...
  if ($magnificPopups.length > 0) {
    var themePluginMagnificPopupInit = function themePluginMagnificPopupInit() {
      ...
      $magnificPopups.each(function () {
        ...
        magnificPopupSettings = jQuery.extend(magnificPopupSettingsDefault, magnificPopupSettingsExtras); // Pass items on a single item ie. a "View Gallery" button
        ...
        $this.magnificPopup(magnificPopupSettings); // Transitions
    ...
    $document.themeLoadPlugin(["magnific-popup/dist/jquery.magnific-popup.min.js"], ["plugin-css/plugin-magnific-popup.min.css", "magnific-popup/dist/magnific-popup.css"], themePluginMagnificPopupInit);
  }
}; // Object of plugins to add to Globals.PLUGINS
...
Globals.PLUGINS.themePluginNiceScroll = function (context) {
  ...
  if ($scrollbars.length > 0) {
    var themePluginCustomScrollbarInit = function themePluginCustomScrollbarInit() {
    ...
    $document.themeLoadPlugin(["https://cdnjs.cloudflare.com/ajax/libs/jquery.nicescroll/3.7.6/jquery.nicescroll.min.js"], [], themePluginCustomScrollbarInit);
  }
}; // Object of plugins to add to Globals.PLUGINS
...
Globals.PLUGINS.themePluginOffCanvas = function (context) {
  ...
  if ($offCanvass.length > 0) {
    var themePluginOffCanvasInit = function themePluginOffCanvasInit() {
      ...
      $('body').wrapInner($bodyWrap); // Move any excluded content outside wrapper (.c-offcanvas-content-wrap)
      ...
      $offCanvass.each(function () {
        ...
        if ($target) {
          ...
          $target.offcanvas(settings); // Toggle bug if already open
    ...
    $document.themeLoadPlugin(["https://unpkg.com/js-offcanvas@1.2.8/dist/_js/js-offcanvas.pkgd.js"], ["https://unpkg.com/js-offcanvas@1.2.8/dist/_css/prefixed/js-offcanvas.css", "plugin-css/plugin-offcanvas.min.css"], themePluginOffCanvasInit);
  }
}; // Object of plugins to add to Globals.PLUGINS
...
Globals.PLUGINS.themePluginOwlCarousel = function (context) {
  ...
  if ($owlCarousels.length > 0) {
    var themePluginOwlCarouselInit = function themePluginOwlCarouselInit(context) {
    ...
    $document.themeLoadPlugin(["https://cdnjs.cloudflare.com/ajax/libs/OwlCarousel2/2.2.1/owl.carousel.min.js"], ["plugin-css/plugin-owl-carousel.min.css", "https://cdnjs.cloudflare.com/ajax/libs/OwlCarousel2/2.2.1/assets/owl.carousel.min.css", "https://cdnjs.cloudflare.com/ajax/libs/animate.css/3.5.2/animate.min.css"], themePluginOwlCarouselInit);
  }
}; // Object of plugins to add to Globals.PLUGINS
...
Globals.PLUGINS.themePluginSliderRevolution = function (context) {
  ...
  if ($sliderRevolutions.length > 0) {
    var themePluginSliderRevolutionInit = function themePluginSliderRevolutionInit() {
      if ($sliderRevolutions.length === 0) {
        $sliderRevolutions = context.find('[data-toggle=slider-rev]');
      }
      ...
      $sliderRevolutions.each(function () {
        ...
        if (customInit) {
          ...
          sliderRevEl.trigger("appstrap:sliderRev:standardInit", [sliderRevSettings]); // Pause sliders on modals open
...
Globals.PLUGINS.themePluginTyped = function (context) {
  ...
  if ($typeds.length > 0) {
    var themePluginTypedInit = function themePluginTypedInit() {
      $typeds.each(function () {
        ...
        if (typedStrings !== '') {
          if (_typeof(typedStrings) === 'object') {
...
Globals.PLUGINS.themePluginVide = function (context) {
  ...
  if ($vides.length > 0) {
    var themePluginVideInit = function themePluginVideInit() {
      $vides.each(function () {
        var videoBg = $(this);
...
Globals.PLUGINS.themePluginZoom = function (context) {
  ...
  if ($zooms.length > 0) {
    var themePluginZoomInit = function themePluginZoomInit() {
      $zooms.each(function () {
        var $this = $(this),
    ...
    $document.themeLoadPlugin(["https://cdnjs.cloudflare.com/ajax/libs/jquery-zoom/1.7.20/jquery.zoom.min.js"], [], themePluginZoomInit);
  }
}; // ===============================================================
...
(function ($) {
  $.extend($.fn, {
    ...
    // ===============================================================
    themeInit: function themeInit(refresh) {
      var context = $(this);
      refresh = refresh || false; // themePreload: Allow custom callbacks from custom-script.js
    ...
    // ===============================================================
    themePluginsLoad: function themePluginsLoad(refresh) {
      var context = $(this);
      ...
      $document = $(document); // Plugin functions
...
$(document).ready(function () {
  "use strict"; // Init theme functions
...
__webpack_require__(/*! /Users/mkozlov/Projects/bugira.com_source/source/_assets/js/main.js */"./source/_assets/js/main.js");
module.exports = __webpack_require__(/*! /Users/mkozlov/Projects/bugira.com_source/source/_assets/sass/main.scss */"./source/_assets/sass/main.scss");
